...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArray = arr.slice(startIdx).join(" ");
  return subArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getPositiveArrayValues(arr) {
  ...
  const positiveArrayValues = [...arr.values()].filter((value) => value > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squaredArray = flattenedArray.map((value) => value * value);
  const filteredArray = squaredArray.filter((value) => value > threshold);
